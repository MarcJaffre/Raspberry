...
func_check_datastore(){ if [ ! -d $DATASTORE ];then mkdir $DATASTORE 2>/dev/null; $BLANK "Creation du dossier"; fi; }
...
func_start_container(){ for i in $(cat /tmp/running);do $BLANK docker start $i ; done; }
# 1>/dev/null 2>/dev/null
##################################################################################################################################
func_volumes_backup(){
 for VOLUME in $(ls /var/lib/docker/volumes | sort -n | grep -v "backingFsBlockDev\|metadata.db");do
  $BLANK docker-volume-snapshot create "$VOLUME" "$DATASTORE"/"$VOLUME".tar;
 done
}

func_volumes_restore(){
 for VOLUME in $(ls $DATASTORE | cut -d "." -f1 | xargs -n1);do
  $BLANK docker-volume-snapshot restore   $DATASTORE/$VOLUME.tar "$VOLUME" 
 done
}
