...
configure_variable() {
     local var_name="$1"
     local var_value="$2"
     read -p "Entrez la nouvelle valeur pour $var_name (actuel: $var_value): " new_value
     if [ -n "$new_value" ]; then
...
mount_share() {
     ...
     if [ "$action" == "m" ]; then
             mount -t cifs //$IP/$SHARE_NAME $MOUNT_POINT -o username=$LOGIN,password=$PASSWORD,version=3.0
             echo "Partage monté."
     elif [ "$action" == "d" ]; then
             umount $MOUNT_POINT
